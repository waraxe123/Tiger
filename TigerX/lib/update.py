        ...
        if changelog:
            ...
            if len(changelog_str) > 4096:
                ...
                await client.send_document(
                    message.chat.id,
                    "output.txt",
                    caption=f"**Type** `.update deploy` **To Update TigerX.**",
                    reply_to_message_id=status.id,
                )
                remove("output.txt")
            else:
                return await status.edit(
                    f"{changelog_str}\n**Type** `.update deploy` **To Update TigerX.**",
...
async def updatees(client, message):
    ...
    for info in repo.iter_commits(f"HEAD..origin/{BRANCH}"):
        updates += f"<b>➣ #{info.count()}: [{info.summary}]({REPO_}/commit/{info}) by -> {info.author}</b>\n\t\t\t\t<b>➥ Commited on:</b> {ordinal(int(dt.fromtimestamp(info.committed_date).strftime('%d')))} {dt.fromtimestamp(info.committed_date).strftime('%b')}, {dt.fromtimestamp(info.committed_date).strftime('%Y')}\n\n"
