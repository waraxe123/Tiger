...
async def api_ninja_dogs(client, message):
    ...
    if not term_param:
        await pro.edit_text("Example: <code>+dog golden retriever</code>")
        return

    if not API_NINJA_DOG:
        await pro.edit_text("Missing api key: <code>API_NINJA_DOG</code>")
        return
    ...
    try:
        this_ninja = await pro.edit_text("<code>Uploading ninja dog.....</code>")
